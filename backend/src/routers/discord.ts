 ...
 * @param {string} guildId - Path parameter representing ID of the guild
 * @returns { BlinkShareRolePosition: number, roles: { id: string, name: string, position: number}[]}
 */
discordRouter.get('/guilds/:guildId/roles', [verifyJwt, checkGuildOwnership], async (req: Request, res: Response) => {
  ...
  try {
    ...
    const BlinkShareRolePosition = roles.find((r) => r.tags?.bot_id === env.DISCORD_CLIENT_ID)?.position;
