        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {sections.map((section, index) => (
            <Card
              key={index}
              className={cn(
                "transition-all duration-200 cursor-pointer hover:shadow-2xl hover:bg-white",
                activeSection === index ? "shadow-2xl bg-slate-20" : "shadow-lg"
              )}
              onClick={() => setActiveSection(index)}
            >
              <CardHeader>
                <div className="flex items-center space-x-4">
                  {/* Render the icon dynamically with custom color */}
