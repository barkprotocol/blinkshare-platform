  ...
  return (
    <div className="p-6">
      ...
      {subscriptions.length > 0 ? (
        <>
          <table className="min-w-full bg-white dark:bg-gray-940">
            <thead>
              <tr>
            ...
            <tbody>
              {currentSubscriptions.map((subscription) => (
                <tr key={subscription.id}>
                  <td className="py-2 px-4 border-b dark:border-gray-200 text-left">
                    {subscription.createTime ? new Date(subscription.createTime).toLocaleString().replace(/,/g, '') : "N/A"}
                  </td>
                  <td className="py-2 px-4 border-b dark:border-gray-200 text-left">{subscription.discordUserId}</td>
                  <td className="py-2 px-4 border-b dark:border-gray-200 text-left">
                    {subscription.expiresAt ? new Date(subscription.expiresAt).toLocaleString().replace(/,/g, '') : "Never"}
                  </td>
                  <td className="py-2 px-4 border-b dark:border-gray-200 text-left">{subscription.role.name}</td>
                  <td className="py-2 px-4 border-b dark:border-gray-200 text-left">{subscription.role.amount}</td>
                </tr>
              ))}
            </tbody>
          </table>
